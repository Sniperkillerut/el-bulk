...
export default function AdminClientDetailPage() {
  ...
  return (
    <div className="flex-1 flex flex-col p-4 md:p-8 max-w-6xl min-h-0 overflow-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
        ...
        <div className="lg:w-96 space-y-8">
          <div className="cardbox p-6">
            <h3 className="text-2xl mb-6 flex items-center gap-2">
              {t('pages.admin.clients.details.order_history', 'ORDER HISTORY')}
              <div className="flex items-center gap-1.5 ml-auto">
                {(detail.orders || []).filter(o => o.status === 'pending').length > 0 && (
            ...
            <div className="space-y-3 max-h-[250px] overflow-y-auto scrollbar-thin pr-1 overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
              {(detail.orders || []).length === 0 ? (
                ...
                (detail.orders || []).map(order => (
                  <Link
                    ...
                    className="block p-4 border border-ink-border/30 hover:border-gold transition-all no-underline group hover:translate-x-1 bg-white rounded-sm"
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase ${order.status === 'completed' ? 'bg-emerald-100 text-emerald-800' :
