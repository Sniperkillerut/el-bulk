      ...
      <section className="bg-kraft-mid border-b-4 border-kraft-dark py-10 md:py-12 px-4 relative overflow-hidden box-lid">
        <div className="centered-container relative z-10 text-center px-4">
          <div className="badge inline-flex mb-4" style={{ background: 'var(--kraft-light)', color: 'var(--ink-deep)', transform: 'rotate(1deg)' }}>
            {t('pages.sealed.landing.category', 'CATEGORY // SEALED')}
          </div>
          <h1 className="font-display text-5xl sm:text-6xl md:text-7xl mb-4" style={{ color: 'var(--ink-deep)' }}>
            {t('pages.sealed.landing.title.main', 'BOXES &')} <span style={{ color: 'var(--gold-dark)' }}>{t('pages.sealed.landing.title.accent', 'PACKS')}</span>
          </h1>
          <p className="text-base md:text-lg max-w-2xl mx-auto" style={{ color: 'var(--text-secondary)' }}>
            {t('pages.sealed.landing.desc', 'From booster boxes to collector packs, explore every set in our vault. Choose your game to see the current inventory.')}
          </p>
        </div>
      </section>
            ...
            >
              <div className="card h-full p-8 flex flex-col items-center justify-center text-center gap-4 border-2 border-kraft-dark transition-shadow hover:shadow-xl relative overflow-hidden">
                ...
                <div>
                  ...
                  <p className="text-xs font-mono-stack mt-1" style={{ color: 'var(--text-muted)' }}>{t('pages.sealed.landing.view_all', 'VIEW ALL SEALED →')}</p>
                </div>

                <div className="absolute -bottom-4 -right-4 opacity-5 -rotate-6 group-hover:opacity-10 transition-opacity">
        ...
        <section>
          <div className="flex items-center gap-4 mb-8">
            <h2 className="font-display text-4xl whitespace-nowrap" style={{ color: 'var(--ink-deep)' }}>
              {t('pages.sealed.landing.featured.main', 'FEATURED')} <span style={{ color: 'var(--text-muted)' }}>{t('pages.sealed.landing.featured.accent', 'SEALED')}</span>
            </h2>
          ...
          {loading ? (
            ...
            <div className="stamp-border rounded-sm p-12 text-center" style={{ color: 'var(--text-muted)' }}>
              <p className="font-display text-2xl mb-2">{t('pages.sealed.landing.no_featured', 'NO FEATURED SEALED FOUND')}</p>
