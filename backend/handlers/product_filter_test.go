...
func TestProductHandler_List_FiltersInactiveTCG(t *testing.T) {
	...
	mock.ExpectQuery("(?i)SELECT COUNT\\(\\*\\) FROM products p").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))

	// 2. Main list query (Note: sqlx expands p.* into explicit columns)
	mock.ExpectQuery("(?i)SELECT .* FROM products p").
	...
	mock.ExpectQuery("(?i)SELECT .* FROM product_stored_in").WillReturnRows(sqlmock.NewRows([]string{"id"}))
	mock.ExpectQuery("(?i)SELECT .* FROM product_categories").WillReturnRows(sqlmock.NewRows([]string{"product_id", "id", "name"}))
