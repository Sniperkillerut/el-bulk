...
BEGIN
    ...
    all_filtered AS (
        SELECT *,
               ...
               (v_card_types_arr IS NULL OR (
                   CASE WHEN p_filter_logic = 'and'
                   THEN (SELECT bool_and(f = ANY(card_types)) FROM unnest(v_card_types_arr) f)
    ...
    f_card_types AS (
        SELECT ct as val, COUNT(*) as c FROM dimension_matches, unnest(card_types) ct
