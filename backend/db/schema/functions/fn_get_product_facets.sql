...
BEGIN
    WITH base_products AS (
        ...
        LEFT JOIN product_category pc_col ON p.id = pc_col.product_id
    ...
    f_condition AS (
        SELECT COALESCE(condition, 'unknown') as val, COUNT(*) as c 
        FROM base_products
    ...
    f_foil AS (
        SELECT COALESCE(foil_treatment, 'unknown') as val, COUNT(*) as c 
        FROM base_products
    ...
    f_rarity AS (
        SELECT COALESCE(LOWER(rarity), 'unknown') as val, COUNT(*) as c 
        FROM base_products
        WHERE (p_condition = '' OR condition = ANY(string_to_array(p_condition, ',')))
    ...
    f_language AS (
        SELECT COALESCE(LOWER(language), 'unknown') as val, COUNT(*) as c 
        FROM base_products
        WHERE (p_condition = '' OR condition = ANY(string_to_array(p_condition, ',')))
          AND (p_foil = '' OR foil_treatment = ANY(string_to_array(p_foil, ',')))
          AND (p_treatment = '' OR card_treatment = ANY(string_to_array(p_treatment, ',')))
    ...
    f_treatment AS (
        ...
        SELECT val, SUM(c) as c FROM (
            SELECT LOWER(card_treatment) as val, COUNT(*) as c 
            FROM base_products 
            WHERE (p_condition = '' OR condition = ANY(string_to_array(p_condition, ',')))
              AND (p_foil = '' OR foil_treatment = ANY(string_to_array(p_foil, ',')))
              AND (p_rarity = '' OR LOWER(rarity) = ANY(string_to_array(p_rarity, ',')))
            ...
            GROUP BY val
            UNION ALL
            SELECT 'full_art' as val, COUNT(*) as c FROM base_products WHERE full_art = true
            UNION ALL
            SELECT 'textless' as val, COUNT(*) as c FROM base_products WHERE textless = true
        ) t GROUP BY val
    ),
    f_collection AS (
        SELECT c_col.slug as val, COUNT(*) as c
        FROM base_products
