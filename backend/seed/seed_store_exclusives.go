...
func seedStoreExclusives(db *sqlx.DB, cats CategoryMap, storage StorageMap) []string {
	...
	seedDeckCards := func(deckID string, identifiers []external.CardIdentifier) {
		...
		for _, identifier := range identifiers {
			...
			r := *res
			qty := 1
			if identifier.CollectorNumber != "" && len(identifier.CollectorNumber) > 2 {
				// Basic lands are qty 4
				// (hack: identify by set+cn pattern; this is just seed data)
			}
			_ = qty
